...
contract CrowdfundingPlatform is ReentrancyGuard {
    ...
    event MilestoneRequested(uint indexed id, uint indexed milestoneId, uint deadline);
    ...
    function getVoterScore(address _voter) public view returns (uint) {
    ...
    function _processLazyScoreUpdate(address _voter) internal {
        ...
        if (m.approved || c.isFailed) {
            ...
            uint currentScore = getVoterScore(_voter);
    ...
    function contribute(uint _id) external payable validCampaign(_id) nonReentrant {
        ...
        if (contributions[_id][msg.sender] == 0) {
            uint score = getVoterScore(msg.sender);
    ...
    function requestMilestoneRelease(uint _id) external validCampaign(_id) {
        ...
        emit MilestoneRequested(_id, campaign.currentMilestone, m.votingDeadline);
    }
    ...
    function voteOnMilestone(uint _id, bool _support) external validCampaign(_id) nonReentrant {
        _processLazyScoreUpdate(msg.sender);
        ...
        // SECURITY FIX: 已移除 tx.origin 限制，現已支援智能錢包與多簽錢包！
    ...
    // SECURITY FIX: 加上 nonReentrant
    function executeRelease(uint _id) external validCampaign(_id) nonReentrant {
        Campaign storage c = campaigns[_id];
    ...
    // SECURITY FIX: 加上 nonReentrant
    function getRefund(uint _id) external validCampaign(_id) nonReentrant {
        Campaign storage c = campaigns[_id];
